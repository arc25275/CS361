...
class AttributeRecord(LoggingHandler):
    def __init__(self, client, theme, attr_id, name, *args, **kwargs):
    ...
    def build_record_option(self, parent, task) -> dict:
        ...
        #  > attr_value
        #  > add_button
        attr_frame = ctk.CTkFrame(parent, bg_color=self.theme["darker"], fg_color=self.theme["darker"])
        ...
        attr_name = ctk.CTkTextbox(attr_frame, font=("Arial", 25), border_width=0, height=1, width=150,
                                   fg_color=self.theme["lighter"], wrap="none", corner_radius=10, )
        ...
        attr_value = ctk.CTkTextbox(attr_frame, font=("Arial", 25), border_width=0, height=1, width=150,
                                    fg_color=self.theme["accent"], wrap="none", corner_radius=10)
        attr_value.insert('1.0', "value")
        attr_value.grid(row=0, column=1, sticky="nsw", pady=10, padx=10)

        add_button = ctk.CTkButton(attr_frame, text="Add", font=("Arial", 25), width=80, bg_color=self.theme["lighter"],
                                   command=lambda: task.add_attribute(self.id, attr_value.get("1.0", "end-1c")))
...
class Attribute(AttributeRecord):
    ...
    def __init__(self, client, theme, attr_id, name, value, task=None, *args, **kwargs):
        super().__init__(client, theme, attr_id, name, *args, **kwargs)
        self.client: Client = client
        self.theme = theme
        self.id: int = attr_id
        self.name: str = name
    ...
    def build_label(self, parent) -> ctk.CTkLabel:
        ...
        attribute_label = ctk.CTkLabel(parent, text=attr_text, font=("Arial", 25),
                                       fg_color=self.theme["lighter"], corner_radius=10, height=50, padx=10, pady=10)
    ...
    def build_current_option(self, parent) -> dict:
        ...
        #  > attr_value
        #  > remove_button
        attr_frame = ctk.CTkFrame(parent, bg_color=self.theme["darker"], fg_color=self.theme["darker"], height=50)
        attr_frame.columnconfigure(0, weight=1)
        attr_frame.columnconfigure(1, weight=1)
        attr_frame.columnconfigure(2, weight=1)

        attr_name = ctk.CTkLabel(attr_frame, text=f'{self.name}', font=("Arial", 25),
                                 fg_color=self.theme["lighter"], corner_radius=10, padx=10, pady=10, height=50)
        attr_name.grid(row=0, column=0, sticky="nsw", pady=10, padx=10)
        attr_value = ctk.CTkTextbox(attr_frame, font=("Arial", 25), border_width=0, height=1, width=150,
                                    fg_color=self.theme["accent"], wrap="none")
        # On Typing, update the attribute
        attr_value.bind("<KeyRelease>", lambda val: self.update_value_temp(attr_value.get("1.0",
                                                                                          "end-1c")))
        attr_value.insert('1.0', f'{self.value}')
        attr_value.grid(row=0, column=1, sticky="nsw", pady=10, padx=10)

        remove_button = ctk.CTkButton(attr_frame, text="Remove", font=("Arial", 25), width=80, bg_color=self.theme["lighter"], fg_color=self.theme["accent"],
                                      command=lambda: self.task.remove_attribute(self.id))
...
class Task(LoggingHandler):
    ...
    def __init__(self, client, theme, task_id, name, date, attributes, description, status, parent=None, children=None,
        ...
        self.options_frame = ctk.CTkFrame(self.detail_view["frame"], bg_color=self.theme["darker"], fg_color=self.theme["darker"])
        self.assign_attributes()
    ...
    def build_list_item(self, parent) -> dict:
        ...
        button = ctk.CTkButton(parent, command=lambda index=self.id: self.client.change_task(index), text="", width=350,
                               height=100, bg_color=self.theme["darker"], fg_color=self.theme["accent"], corner_radius=10)
        if self.status == "closed":
            button.configure(bg_color=self.theme["darker"], fg_color=self.theme["lighter"])
        number = f"{self.id + 1}"
        ...
        name = ctk.CTkLabel(button, text=f'{number}: {self.date} - {self.name}', font=("Arial", 20), padx=10, pady=10,
                            fg_color="transparent", bg_color="transparent", text_color=self.theme["font_alt"])
        ...
        filler = ctk.CTkLabel(button, text="", padx=10, text_color=self.theme["font_alt"])
        if self.status == "closed":
        ...
        # Attributes for task
        attr_list = ", ".join(attr.value for attr in self.attributes)
        attributes = ctk.CTkLabel(button, text=f'{attr_list}', font=("Arial", 20), padx=10, pady=10, text_color=self.theme["font_alt"])
    ...
    def build_detail_view(self, parent):
        ...
        task_detail_frame = ctk.CTkFrame(parent, bg_color=self.theme["darker"], fg_color=self.theme["darker"], height=800)
        ...
        # Name Frame
        name_frame = ctk.CTkFrame(task_detail_frame, bg_color=self.theme["darker"], fg_color=self.theme["darker"])
        name_frame.grid(row=0, column=0, sticky="nsw")
        name_frame.columnconfigure(0, weight=1)
        name_frame.columnconfigure(1, weight=1)

        # Name Label
        name_label = ctk.CTkLabel(name_frame, text=f'Task {self.id + 1} - ', font=("Arial", 30), text_color=self.theme["font"])

        # Name Textbox
        task_name = ctk.CTkTextbox(name_frame, font=("Arial", 30), border_width=0,
                                   height=1, width=300, fg_color=self.theme["darker"], wrap="none", text_color=self.theme["font"])
        ...
        # Completion Checkbox
        complete_frame = ctk.CTkFrame(task_detail_frame, fg_color=self.theme["darker"], width=100)
        complete_text = ctk.CTkLabel(complete_frame, text="Complete ", font=("Arial", 30), text_color=self.theme["font"])
        ...
        # Delete Button
        delete_button = ctk.CTkButton(task_detail_frame, text="Delete",
                                      command=lambda: self.client.delete_task(self.id),
                                      font=("Arial", 30), width=80, bg_color=self.theme["darker"], fg_color=self.theme["accent"], state="disabled")
        if self.status == "closed":
        ...
        edit_button = ctk.CTkButton(task_detail_frame, text="Edit", command=lambda: self.client.edit_task(self.id),
                                    font=("Arial", 30), width=80, bg_color=self.theme["darker"], fg_color=self.theme["accent"])
        edit_button.grid(column=2, row=0, sticky="nsw", pady=10)

        # Date
        date = ctk.CTkTextbox(task_detail_frame, font=("Arial", 30), border_width=0, height=1, wrap="none", fg_color=self.theme["accent"])
    ...
    def build_attribute_options(self, attr_options_frame):
        ...
        max_i += 1
        new_attr_frame = ctk.CTkFrame(attr_options_frame, bg_color=self.theme["darker"], fg_color=self.theme["darker"])
        ...
        add_button = ctk.CTkButton(new_attr_frame, text="Add", font=("Arial", 25), width=80, bg_color=self.theme["darker"],
                                   corner_radius=10)
    ...
    def create_attribute(self, name: str, value: str) -> Attribute | None:
        ...
        task_response = self.client.server.post(f"tasks/{self.id}/attributes",
                                                {"id": attr_id, "name": name, "value": value})

        if task_response["code"] == 200 and attr_response["code"] == 200:
            self.attributes.append(new_attribute)
            # Add label to task detail view
            new_attribute.label.grid(row=self.detail_view["attr_row"], column=0, columnspan=3, sticky="nsw", pady=10,
                                     padx=10)
            self.detail_view["attr_row"] += 1
            # Add to current options
            self.attribute_options["current_options"].append(new_attribute.option)
            new_attribute.option["frame"].grid(row=self.attribute_options["current_row"], column=0, columnspan=3,
                                               sticky="nsw", pady=10, padx=10)
    ...
    def add_attribute(self, attr_id: int, value: str) -> Attribute | None:
        ...
        if response["code"] == 200:
            ...
            existing_option = next(
                (opt for opt in self.attribute_options["existing_options"] if list(opt.keys())[0] == attr_id), None)
    ...
    def remove_attribute(self, attr_id: int) -> bool:
        ...
        if response["code"] == 200:
            ...
            record.record["frame"].grid(row=self.attribute_options["existing_row"], column=0, columnspan=3,
                                        sticky="nsw", pady=10, padx=10)
...
class Client(LoggingHandler):
    ...
    def get_theme(self):
        ...
        choice = ""
        with open("../microservice_B/data.json", "r") as file:
            data = json.load(file)
            choice = data["theme"]
        if choice == "default":
            theme = {"font": "#FFFFFF", "font_alt": "#FFFFFF", "lighter": "gray20", "darker": "gray14", "accent": "royal blue"}
        elif choice == "random":
            theme = self.theme_server.get_theme("colors")
        else:
            theme = self.theme_server.get_theme(choice)
        return theme

    def change_theme(self, choice):
        """Change the theme in the Json File"""
        with open("../microservice_B/data.json", "r") as file:
            data = json.load(file)
            data["theme"] = choice
        with open("../microservice_B/data.json", "w") as file:
            json.dump(data, file)
        self.log.info(f"Theme changed to {choice}")
    ...
    def build_menu(self):
        ...
        sorting_button = ctk.CTkButton(menu_bar, text="Sort and Filter", font=("Arial", 20), width=40, height=20,
                                       command=self.toggle_sf_menu, fg_color=self.theme["accent"], bg_color="gray14", text_color=self.theme["font_alt"])
        ...
        add_task_button = ctk.CTkButton(menu_bar, text="Add Task", font=("Arial", 20), width=20, height=20,
                                        command=self.add_task, fg_color=self.theme["accent"], bg_color="gray14", text_color=self.theme["font_alt"])
        add_task_button.grid(row=0, column=2, sticky="nsw", pady=10, padx=10)


        # Changing Theme
        theme_frame = ctk.CTkFrame(menu_bar, bg_color="gray14", fg_color="gray17")
        theme_frame.grid(row=0, column=3, sticky="nsw", pady=10, padx=10)
        theme_label = ctk.CTkLabel(theme_frame, text="Theme: ", font=("Arial", 20), text_color=self.theme["font_alt"])
        theme_label.grid(row=0, column=0, sticky="nsw")
        # Default is what I originally chose, Random is from Microservice A
        theme_picker = ctk.CTkComboBox(theme_frame, values=["Default", "Random"], width=100)
        theme_picker.grid(row=0, column=1, sticky="nsw")
        theme_button = ctk.CTkButton(theme_frame, text="Change", font=("Arial", 20), width=10, height=1,
    ...
    def build_root(self, root: ctk.CTk):
        ...
        root.configure(background=self.theme["darker"])
    ...
    def build_task_list_container(self):
        ...
        left_canvas = ctk.CTkCanvas(task_container, bg=self.theme["darker"], highlightthickness=0)
        ...
        task_list_container = ctk.CTkFrame(left_canvas, bg_color=self.theme["darker"], fg_color=self.theme["darker"])
    ...
    def build_detail_container(self):
        ...
        #      > task_detail_frame[] [[task_detail_frame]]
        #  > right_scrollbar
        detail_container = ctk.CTkFrame(self.root, bg_color=self.theme["darker"], fg_color=self.theme["darker"])
        ...
        # Create a canvas so that it can be scrollable
        right_canvas = ctk.CTkCanvas(detail_container, bg=self.theme["darker"], highlightthickness=0)
        ...
        extra_space = ctk.CTkLabel(task_detail_container, text="", bg_color=self.theme["darker"], fg_color=self.theme["darker"], height=500)
    ...
    def build_help_page(self) -> ctk.CTkFrame:
        ...
        help_page = ctk.CTkFrame(self.detail_container, bg_color=self.theme["darker"], fg_color=self.theme["darker"])
        ...
        # New Info
        new_title = ctk.CTkLabel(help_page, text="What's New?", font=("Arial", 30), bg_color=self.theme["darker"], text_color=self.theme["font"],
                                 fg_color=self.theme["darker"],
                                 height=2)
        new_title.grid(row=0, column=0, sticky="nsw", padx=10, pady=10)
        new_info = ctk.CTkTextbox(help_page, font=("Arial", 20), bg_color=self.theme["darker"], fg_color=self.theme["darker"], text_color=self.theme["font"],
        ...
        # Help
        help_title = ctk.CTkLabel(help_page, text="Help", font=("Arial", 30), bg_color=self.theme["darker"], fg_color=self.theme["darker"], text_color=self.theme["font"],
                                  height=2)
        help_title.grid(row=2, column=0, sticky="nsw", padx=10, pady=10)
        help_info = ctk.CTkTextbox(help_page, font=("Arial", 20), bg_color=self.theme["darker"], fg_color=self.theme["darker"], text_color=self.theme["font"],
    ...
    def build_sf_menu(self):
        ...
        sf_menu = ctk.CTkFrame(self.menu_bar["menu_bar"], bg_color="gray14", fg_color=self.theme["darker"], corner_radius=10)
        sort_menu = ctk.CTkFrame(sf_menu, bg_color="gray14", fg_color=self.theme["darker"], corner_radius=10)
        sort_menu.grid(row=0, column=0, sticky="nsw")
        filter_menu = ctk.CTkFrame(sf_menu, bg_color="gray14", fg_color=self.theme["darker"], corner_radius=10)
        ...
        sort_label = ctk.CTkLabel(sort_menu, text="Sort By", font=("Arial", 20), padx=10, pady=10, text_color=self.theme["font"])
        ...
        sort_picker = ctk.CTkComboBox(sort_menu, font=("Arial", 20), width=150, height=20,
                                      values=["None", "id", "name", "date", "status"], command=sort_picker_callback, fg_color=self.theme["lighter"], bg_color=self.theme["darker"], text_color=self.theme["font"], border_color="gray14", button_color=self.theme["accent"])
        ...
        sort_picker_attr = ctk.CTkComboBox(sort_menu, font=("Arial", 20), width=150, height=20,
                                           values=["None"] + attr_record_list, command=sort_picker_callback, fg_color=self.theme["lighter"], bg_color=self.theme["darker"], text_color=self.theme["font"], border_color="gray14", button_color=self.theme["accent"])
        ...
        sort_attr = ctk.CTkButton(sort_menu, text="Sort by Attribute", font=("Arial", 20), width=20, height=20,
                                  command=toggle_sort_attr, fg_color=self.theme["accent"], bg_color=self.theme["darker"], text_color=self.theme["font_alt"])
        sort_attr.grid(row=1, column=0, sticky="nsw", pady=10, padx=10)

        order_label = ctk.CTkLabel(sort_menu, text="Order: Asc", font=("Arial", 20), padx=10, pady=10, text_color=self.theme["font"])
        ...
        order_button = ctk.CTkButton(sort_menu, text="Toggle Order", font=("Arial", 20), width=20, height=20,
                                     command=toggle_order, fg_color=self.theme["accent"], bg_color=self.theme["darker"], text_color=self.theme["font_alt"])
        ...
        filter_label = ctk.CTkLabel(filter_menu, text="Filter By", font=("Arial", 20), padx=10, pady=10, text_color=self.theme["font"])
        ...
        filter_picker = ctk.CTkComboBox(filter_menu, font=("Arial", 20), width=150, height=20,
                                        values=["None", "id", "name", "date", "status"], command=filter_picker_callback, fg_color=self.theme["lighter"], bg_color=self.theme["darker"], text_color=self.theme["font"], border_color="gray14", button_color=self.theme["accent"])
        filter_picker.grid(row=0, column=1, sticky="nsw", pady=10, padx=10)

        filter_picker_attr = ctk.CTkComboBox(filter_menu, font=("Arial", 20), width=150, height=20,
                                             values=["None"] + attr_record_list, command=filter_picker_callback, fg_color=self.theme["lighter"], bg_color=self.theme["darker"], text_color=self.theme["font"], border_color="gray14", button_color=self.theme["accent"])
        ...
        filter_attr = ctk.CTkButton(filter_menu, text="Filter by Attribute", font=("Arial", 20), width=20, height=20,
                                    command=toggle_filter_attr, fg_color=self.theme["accent"], bg_color=self.theme["darker"], text_color=self.theme["font_alt"])
        filter_attr.grid(row=1, column=0, sticky="nsw", pady=10, padx=10)

        value_label = ctk.CTkLabel(filter_menu, text="Value", font=("Arial", 20), padx=10, pady=10, text_color=self.theme["font"])
        value_label.grid(row=2, column=0, sticky="nsw")

        value_entry = ctk.CTkTextbox(filter_menu, font=("Arial", 20), width=150, height=20, fg_color=self.theme["lighter"], text_color=self.theme["font"])
        ...
        value_button = ctk.CTkButton(filter_menu, text="Filter", font=("Arial", 20), width=20, height=20,
                                     command=filter_value, fg_color=self.theme["accent"], bg_color=self.theme["darker"], text_color=self.theme["font_alt"])
    ...
    def build_task_list(self):
        ...
        if self.filter["filter"] != "":
            filtered_ids = self.sort_server.filter_tasks(self.filter["filter"], self.filter["value"],
                                                         self.filter["attr"])
        ...
        if response["code"] == 200:
            for task in response["data"]:
                ...
                for attr in task["attributes"]:
                    attr_list.append(Attribute(self, self.theme, attr["id"], attr["name"], attr["value"]))
                new_task = Task(self, self.theme, task["id"], task["name"], task["date"], attr_list,
                                task["description"], task["status"], task["parent"], task["children"])
    ...
    def fetch_attributes(self) -> bool:
        ...
        if response["code"] == 200:
            for attr in response["data"]:
                new_attribute = AttributeRecord(self, self.theme, attr["id"], attr["name"])
    ...
    def edit_task(self, n):
        ...
        if task.editing is True:
            task.editing = False
            task.detail_view["name"].configure(state="disabled", fg_color=self.theme["darker"])
            task.detail_view["date"].configure(state="disabled", fg_color=self.theme["accent"])
            task.detail_view["description"].configure(state="disabled", fg_color="gray12")
            task.detail_view["attributes"].configure(state="disabled", fg_color=self.theme["darker"])
            task.detail_view["frame"].configure(bg_color=self.theme["darker"], fg_color=self.theme["darker"])
            task.detail_view["frame"].grid(row=1, column=1, columnspan=3, sticky='new')
            ...
            task.editing = True
            task.detail_view["edit"].configure(state="normal")
            task.detail_view["name"].configure(state="normal", fg_color=self.theme["accent"])
            task.detail_view["name"].focus_set()
            task.detail_view["date"].configure(state="normal", fg_color=self.theme["accent"])
            task.detail_view["description"].configure(state="normal", fg_color=self.theme["accent"])
            task.detail_view["attributes"].configure(state="normal", fg_color=self.theme["accent"])
            task.detail_view["frame"].configure(bg_color=self.theme["lighter"], fg_color=self.theme["lighter"])
            task.detail_view["frame"].grid(row=1, column=1, columnspan=3, sticky='new')
    ...
    def toggle_active(self, n: int):
        ...
        task.toggle_active()
        if task.status == "open":
            task.list_item["button"].configure(fg_color=self.theme["accent"])
            task.list_item["filler"].configure(text="", font=("Arial", 20), fg_color=self.theme["accent"])
            task.list_item["name"].configure(fg_color=self.theme["accent"])
            task.list_item["attributes"].configure(fg_color=self.theme["accent"])
            task.detail_view["delete"].configure(state="disabled")
        else:
            task.list_item["button"].configure(bg_color=self.theme["darker"], fg_color=self.theme["lighter"])
            task.list_item["filler"].configure(text="✓", font=("Arial", 20), bg_color=self.theme["lighter"], fg_color=self.theme["lighter"])
            task.list_item["name"].configure(bg_color=self.theme["lighter"], fg_color=self.theme["lighter"])
            task.list_item["attributes"].configure(bg_color=self.theme["lighter"], fg_color=self.theme["lighter"])
    ...
    def add_child(self, id: int):
        ...
        if response["code"] != 200:
            self.log.error(
                f"Error adding child {child.id} to parent {parent.id}: {response["code"]} : {response["message"]}")
...
class Connection(LoggingHandler):
    ...
    def get_theme(self, type: str) -> dict:
        ...
        :param type: The type of theme to get. Can be "colors", "animal" or "nature"
        :return: The theme for the application
        """
        data = {
            "type": "theme",
            "theme": type
        ...
        self.socket.send_string(json.dumps(data))
        response = json.loads(self.socket.recv_string())
        self.log.debug(f"Got theme: {response}")
